...
#if defined(NDEBUG)
#error "SCM cannot be compiled without assertions."
#endif
...
{
    //~43200 blocks / month
    //129600 blocks / 3 months
    //259200 blocks / 6 months
    ...
    //Buffer period
    //Hardcode your mom, if you want hardcode...
    } else if (nHeight > 0 && nHeight <= Params().LAST_POW_BLOCK()) {   
        nSubsidy = 0.01 * COIN;
    //0.5 SCM / block in the first month
    } else if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 43200) {
        nSubsidy = 0.5 * COIN;
    // 5 SCM / block between the first month and the 3rd month
    } else if (nHeight > 43200 && nHeight <= 129600) {
        nSubsidy = 5 * COIN;
    // 7 SCM / block after 3 months, for another 3 months, the Masternode collateral should increase in the 3rd month
    } else if (nHeight > 129600 && nHeight <= 259200) {
        nSubsidy = 7 * COIN;
    // 10 SCM / block after 6 months, for another 6 months, the Masternode collateral should increase in the 6th month
    } else if (nHeight > 259200 && nHeight <= 518400) {
        nSubsidy = 10 * COIN;
    // Start the halving after 1 year, each 6 months
    } else if (nHeight > 518400 && nHeight <= 777600) {
        nSubsidy = 5 * COIN;
    //Increase the MN collateral
    } else if (nHeight > 777600 && nHeight <= 1036800) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight > 1036801) {
        nSubsidy = 1 * COIN;
    }
...
{
    RenameThread("simulacrum-scriptch");
...
bool RecalculateSCMSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating SCM supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating SCM supply..."), percent);
        }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    LogPrintf("GetBlockValue(pindex->pprev->nHeight): %s , %s\n", GetBlockValue(pindex->pprev->nHeight), pindex->pprev->nHeight);
    LogPrintf("GetBlockValue(pindex->nHeight): %s , %s \n", GetBlockValue(pindex->nHeight), pindex->nHeight);
...
bool CheckColdStakeFreeOutput(const CTransaction& tx, const int nHeight)
{
    ...
    if (outs >=3 && lastOut.scriptPubKey != tx.vout[outs-2].scriptPubKey) {
        ...
        // cold staking is active much after nPublicZCSpends so GetMasternodePayment is always 3 SCM.
    ...
    if (isPoS) {
        ...
        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                ...
                scmInputs.push_back(stakeIn);
            }
        }
        const bool hasSCMInputs = !scmInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasSCMInputs)
                    ...
                    for (const CTxIn& pivIn : scmInputs){
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : scmInputs) {
