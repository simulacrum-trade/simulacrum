...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (simulacrum.conf).");
            QMessageBox::warning(this, "Simulacrum Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
