...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SCM address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // Convert simulacrum:// to simulacrum:
    //
    //    Cannot handle this later, because simulacrum:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    ...
    /* Open simulacrum.conf with the associated application */
...
{
    return GetAutostartDir() / "simulacrum.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a simulacrum.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the simulacrum app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add simulacrum app to startup item list
