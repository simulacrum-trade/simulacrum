...
void CheckOffsetDisconnectedPeers(const CNetAddr& ip)
{
    ...
    if (setOffsetDisconnectedPeers.size() >= MAX_TIMEOFFSET_DISCONNECTIONS) {
        ...
        // Trigger the warning
        std::string strMessage = _("Warning: Peers are being disconnected due time differences. Please check that your computer's date and time are correct! If your clock is wrong Simulacrum will not work properly.");
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "Simulacrum " + FormatFullVersion();
...
bool BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Simulacrum is probably already running."), addrBind.ToString());
        else
