...
In a typical situation, this suffices to run behind a Tor proxy:
	...
	./simulacrumd -proxy=127.0.0.1:9050
...
versions of Tor see [Section 3](#3-automatically-listen-on-tor).*
	...
	HiddenServiceDir /var/lib/tor/simulacrum-service/
	HiddenServicePort 59472 127.0.0.1:59472
	HiddenServicePort 61472 127.0.0.1:61472

The directory can be different of course, but (both) port numbers should be equal to
your simulacrumd's P2P listen port (59472 by default).

	-externalip=X   You can tell simulacrum about its publicly reachable address using
	                this option, and this can be a .onion address. Given the above
	                configuration, you can find your .onion address in
	                /var/lib/tor/simulacrum-service/hostname. For connections
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./simulacrumd -proxy=127.0.0.1:9050 -externalip=simulacrumzj6l4cvo2fxy.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

	./simulacrumd ... -bind=127.0.0.1

If you don't care too much about hiding your node, and want to be reachable on IPv4
as well, use `discover` instead:

	./simulacrumd ... -discover

and open port 59472 on your firewall (or use -upnp).

If you only want to use Tor to reach .onion addresses, but not use it as a proxy
for normal IPv4/IPv6 communication, use:

	./simulacrumd -onion=127.0.0.1:9050 -externalip=simulacrumzj6l4cvo2fxy.onion -discover
...
in `torrc` config file. For cookie authentication the user running simulacrumd must have read
access to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running simulacrumd to the same group and setting permissions appropriately. On
Debian-based systems the user running simulacrumd can be added to the debian-tor group,
