...
- [Building PIVX Core](#building-simulacrum-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/simulacrum-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/simulacrum-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/simulacrum-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/simulacrum-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/simulacrum-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/simulacrum-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/simulacrum-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@simulacrum.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/simulacrum-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/simulacrum-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/simulacrum-osx-*-build.assert
```
