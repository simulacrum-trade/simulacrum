...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='simulacrum.conf'
CONFIGFOLDER='/root/.simulacrum'
COIN_DAEMON='simulacrumd'
COIN_CLI='simulacrum-cli'
COIN_PATH='/usr/local/bin/'
COIN_PATH_MOVE='/usr/local/'
COIN_TGZ='https://github.com/simulacrum-trade/simulacrum/releases/download/v1.2.0/simulacrum-1.2.0-x86_64-linux-gnu.tar.gz'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='simulacrum'
COIN_PORT=59472
RPC_PORT=59473
FOLDER_SRC=simulacrum-1.2.0
...
function sync_node() {
  ...
  rm -r {blocks,database,fee_estimates.dat,mnpayments.dat,simulacrumd.pid,budget.dat,db.log,peers.dat,chainstate,debug.log,mncache.dat} >/dev/null 2>&1
